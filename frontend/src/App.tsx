...
import AnchorCategoryDonut from "./components/charts/AnchorCategoryDonut";
...
function Dashboard() {
  ...
  const [ovBroken, setOvBroken] = useState<BrokenLinksSummary | null>(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [ovSitewide, setOvSitewide] = useState<any>(null);
  const [ovRedirects, setOvRedirects] = useState<RedirectSummary | null>(null);
  const [ovAnchorCats, setOvAnchorCats] = useState<Record<string, number>>({});
  const [ovPageCats, setOvPageCats] = useState<PageBreakdownResponse["categories"]>({});
  const [ovTopPages, setOvTopPages] = useState<PageRow[]>([]);
  const [ovTopDomains, setOvTopDomains] = useState<ReferringDomain[]>([]);
  ...
  useEffect(() => {
    ...
    if (tab === "overview") {
      ...
      fetchBrokenLinks(selected, { per_page: 1 }).then((r) => setOvBroken(r.summary)).catch(() => setOvBroken(null));
      fetchSitewide(selected).then(setOvSitewide).catch(() => setOvSitewide(null));
      fetchRedirects(selected).then(setOvRedirects).catch(() => setOvRedirects(null));
      fetchAnchors(selected).then((r) => setOvAnchorCats(r.categories ?? {})).catch(() => setOvAnchorCats({}));
      fetchPageBreakdown(selected).then((r) => { setOvPageCats(r.categories ?? {}); setOvTopPages(r.pages ?? []); }).catch(() => { setOvPageCats({}); setOvTopPages([]); });
      fetchReferringDomains(selected, { sort: "max_dr:desc" }).then(setOvTopDomains).catch(() => setOvTopDomains([]));
    } else if (tab === "links") {
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <AlertCard
                  label="Broken Links"
                  value={ovBroken ? ovBroken.total_broken.toLocaleString() : "—"}
                  subtitle={ovBroken ? `4xx: ${ovBroken.count_4xx} / 5xx: ${ovBroken.count_5xx}` : undefined}
                  status={!ovBroken || ovBroken.total_broken === 0 ? "ok" : ovBroken.total_broken <= 10 ? "warning" : "danger"}
                  tooltip={METRICS.broken_links_alert.short}
                  onClick={() => handleTabClick("broken")}
                />
                <AlertCard
                  label="Sitewide Links"
                  value={(() => {
                    const sw = ovSitewide?.aggregate?.sitewide ?? ovSitewide?.with_sitewide;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <AnchorCategoryDonut categories={ovAnchorCats} />
              <PageCategoryChart
                data={Object.entries(ovPageCats).map(([category, stats]) => ({ category, link_count: stats.link_count }))}
              />
            </div>

            {/* DR Distribution & Velocity (existing) */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <TopDomainsTable domains={ovTopDomains} onViewAll={() => handleTabClick("domains")} />
              <TopPagesTable pages={ovTopPages} onViewAll={() => handleTabClick("pages")} />
            </div>
          </div>
        )}
